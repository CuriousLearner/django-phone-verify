...
class BaseBackend(metaclass=ABCMeta):
    ...
    def validate_security_code(self, security_code, phone_number, session_token):
        ...
        if stored_verification is None:
            # Allow sandbox backends to bypass when no verification record exists (for testing)
        ...
        # check if too many failed attempts (this must be checked before sandbox bypass)
        ...
        # Allow sandbox backends to bypass code check for testing (but after brute force check)
        if self._should_bypass_code_check(security_code):
